...
function decrypt(data, password, iterations, success, error, backup_found_cb) {
...
function check_password(encrypted, password, iterations) {
	var	found = false;
	var iterations=iterations || 0;
	try {
		decrypt(encrypted, password, iterations, function (decrypted) {
			try {
			...
			if (obj.double_encryption) {
				...
				for (var i = 0; i < obj.keys.length; ++i) {
					obj.keys[i].priv = decrypt(obj.keys[i].priv, obj.sharedKey + pass, iterations, isBase58);
				}
